...
* Author Email: arjun@mobiwebtech.com
* version: 1.0
*/


/*
* Function Name : is_deactive
* purpose       : To check whether user is active or not
* Auther        : Arjun
*/
  function is_deactive($user_id){
    $CI = & get_instance();
    $user_id = $CI->security->xss_clean($user_id);
    $userdata = $CI->common_model->getsingle(USERS,array('id'=>$user_id,'active != '=>1));
    if(!empty($userdata)){
      echo '{"error":true,"status":-1,"error_message":"User is deactive","data":{}}';die;
    }
  }
...
if(! function_exists('save_user_notification')){
  function save_user_notification($type_id,$user_id,$noti_type,$type,$message,$title=''){
    $CI = & get_instance();
      $notification_arr = array(
      // In case of order, it(type_id) is order id. 
      // In case of offer it(type_id) is offer id      

        'type_id' => $type_id, 
        'sender_id' => ADMIN_ID,
        'reciever_id' => $user_id,
        'notification_type' => $noti_type,
        'type' => $type, //1-al-a cart,2-foodparcel,3-partypackage
        'title' => $title,
        'message' => $message,
        'is_read' => 0,
        'is_send' => 0,
      ...
      $lid = $CI->common_model->insertData(USER_NOTIFICATION,$notification_arr);
      if($lid)
      {
        return TRUE;
      }else{
...
if ( ! function_exists('send_push_notifications')) {
  function send_push_notifications($msg,$user_id,$params)
  {
    ...
    if(!empty($friend_details) && $friend_details->is_blocked == 0 && $friend_details->is_logged_out == 0){
      ...
      if(!empty($device_history))
      {
        ...
        //arjun
        $device_badges = (int) $CI->common_model->get_user_badges($user_id);

        $CI->common_model->updateFields(USERS, array('badges' => $device_badges), array('id' => $user_id));

        /* Send notification to all devices */
      
        foreach($device_history['result'] as $dh)
        {
...
if (!function_exists('Execute_Curl_URL'))
{

    function Execute_Curl_URL($url,$postfields)
    {
...
if (!function_exists('fileUpload')) {
    ...
    function fileUpload($filename, $subfolder, $ext, $size = "", $width = "", $height = "") {
        ...
        $tempArr = explode('.', $_FILES[$filename]['name']);
        $extn = array_pop($tempArr);

        $config['upload_path'] = 'uploads/' . $subfolder . '/';
        $config['allowed_types'] = $ext;
        $config['file_name'] = 'Img_'.uniqid().'_'.time().'_.'.$extn;
...
//This will return random string of given length.
